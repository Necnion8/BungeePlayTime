...
public class SQLiteDatabase implements Database {
    ...
    public SQLiteDatabase(File filename, Map<String, String> dbOptions, Logger logger) {
        this.filename = filename;
        this.log = logger;
        this.dbOptions = dbOptions;
    }

    public SQLiteDatabase(File parentDirectory, MainConfig.Database settings, Logger logger) {
        this(new File(parentDirectory, settings.getAddress()), settings.getOptions(), logger);
    }
    ...
    public Optional<PlayerTimeResult> lookupTime(UUID playerId, LookupTimeOptions options) throws SQLException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            if (options.getServerName().isPresent())
                stmt.setString(pIndex, options.getServerName().get());
